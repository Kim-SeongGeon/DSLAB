...
- [X] UGV(ROS2 Humble, Ubuntu) ↔ GCS(Windows) 간 브리지 경로 구성 및 검증
...
- ROS2(Ubuntu) → Bridge(Windows) → Web-based GCS로 이어지는 엔드-투-엔드 위치 데이터 갱신에 성공
...
- 또한, 기존에 확보해둔 자율주행 차량을 점검한 결과, 하드웨어 일부 손상 및 운영체제가 ROS2 환경과 호환되지 않음을 확인함.
...
- 구매할 로봇은 ROS2 기반 개발 환경에 최적화되어 있어 향후 테스트와 확장성 면에서 유리할 것으로 판단됨
